    ...
    char *hostname;
{
	...
	*/
    ...
    addr = MAP_ADDR;
    kr = vm_map(mach_task_self(), &addr, size, 0, FALSE, memory_object, 0, FALSE,
...
slave_usage()
{
    fprintf(stderr, "usage: slave [spin | check | read | write] hostname size(KB)\n");
    exit(1);
}
...
stats_usage()
{
    fprintf(stderr, "Netmemory statistics not implemented on Mach 3.0\n");
    exit(1);
}
    ...
    char **argv;
{
    ...
    memory_object = (mach_port_t) map_object(0, argv[1]);
